<template>
	<div v-if="permsArray.includes('payment_modes_view') || permsArray.includes('admin')">
		<a-tooltip
			v-if="tooltip"
			placement="topLeft"
			:title="$t('payment_mode.add')"
		...
		<AddEdit
			...
			:formData="formData"
			:data="formData"
			:pageTitle="$t('payment_mode.add')"
			:successMessage="$t('payment_mode.created')"
		/>
	</div>
</template>
