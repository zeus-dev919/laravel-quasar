<template>
	<div
		v-if="permsArray.includes('custom_fields_create') || permsArray.includes('admin')"
	>
		<a-tooltip
			v-if="tooltip"
			placement="topLeft"
			:title="$t('custom_field.add')"
		...
		<AddEdit
			...
			:formData="formData"
			:data="formData"
			:pageTitle="$t('custom_field.add')"
			:successMessage="$t('custom_field.created')"
		/>
	</div>
</template>
